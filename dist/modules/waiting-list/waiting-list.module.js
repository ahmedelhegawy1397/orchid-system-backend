"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WaitingListModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const waiting_list_item_schema_1 = require("./schemas/waiting-list-item.schema");
const waiting_list_service_1 = require("./waiting-list.service");
const waiting_list_controller_1 = require("./waiting-list.controller");
const patients_module_1 = require("../patients/patients.module");
let WaitingListModule = class WaitingListModule {
};
exports.WaitingListModule = WaitingListModule;
exports.WaitingListModule = WaitingListModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([{ name: waiting_list_item_schema_1.WaitingListItem.name, schema: waiting_list_item_schema_1.WaitingListItemSchema }]),
            (0, common_1.forwardRef)(() => patients_module_1.PatientsModule),
        ],
        controllers: [waiting_list_controller_1.WaitingListController],
        providers: [waiting_list_service_1.WaitingListService],
        exports: [waiting_list_service_1.WaitingListService, mongoose_1.MongooseModule],
    })
], WaitingListModule);
//# sourceMappingURL=waiting-list.module.js.map